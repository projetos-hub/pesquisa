...
```bash
# Windows
tasklist | grep -i Codex | grep -v grep
# Linux/Mac
ps aux | grep Codex | grep -v grep
```

- Se > 1 instancia Codex → **WARN IMEDIATO**
- Listar PIDs encontrados
- **NAO prosseguir com file writes ate usuario confirmar**
- Causa: race condition em .Codex.json (problema recorrente documentado)

### Check 2: CONFIG VALIDATION

- Ler `.Codex.json` (se existir)
- Validar JSON (tentar parse)
- Se corrupto:
  1. Verificar `.Codex.json.bak` existe
...
- Se encontrar `.Codex/worktrees/` abandonados → reportar
...
- Criar backup timestamped de `.Codex.json`:
  ```
  .Codex-backups/Codex-YYYY-MM-DD-HHmm.json
  ```
...
[BACKUP]     CREATED (Codex-2026-02-28-1530.json)
